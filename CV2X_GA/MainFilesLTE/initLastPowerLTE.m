...
if ~isempty(stationManagement.transmittingIDsLTEGA) 
    %当前时刻各车辆之间的接收功率
    RXpower_MHz_ofLTEGA = sinrManagement.P_RX_MHzGA;

    % Number of vehicles transmitting in the current subframe
    NtxGA = length(stationManagement.transmittingIDsLTEGA);

    sinrManagement.neighPowerUsefulLTEGA = zeros(NtxGA,length(stationManagement.activeIDsLTE)-1);
    sinrManagement.neighPowerInterfDataLTEGA = zeros(NtxGA,length(stationManagement.activeIDsLTE)-1);
    sinrManagement.neighPowerInterfControlLTEGA = zeros(NtxGA,length(stationManagement.activeIDsLTE)-1);
    
    sinrManagement.neighborsSINRaverageLTEGA = zeros(NtxGA,length(stationManagement.activeIDsLTE)-1);
    sinrManagement.neighborsSINRsciAverageLTEGA = zeros(NtxGA,length(stationManagement.activeIDsLTE)-1);

    % Find not assigned BRid
    indexNOT = (stationManagement.BRidGA<=0);

    % Calculate BRidT = vector of BRid in the time domain
    BRidTGA = ceil(stationManagement.BRidGA ./appParams.NbeaconsF);
    BRidTGA(indexNOT) = -1;

    % Calculate BRidF = vector of BRid in the frequency domain
    BRidFGA = mod(stationManagement.BRidGA-1,appParams.NbeaconsF)+1;
    BRidFGA(indexNOT) = -1;

    for i_tx = 1:NtxGA   %对于每一个发射车辆

        % Find BRT and BRF in use by tx vehicle i
        BRTtx = BRidTGA(stationManagement.transmittingIDsLTEGA(i_tx));
        BRFtx = BRidFGA(stationManagement.transmittingIDsLTEGA(i_tx));

        indexNeighborOfVehicleTX = find(stationManagement.neighborsIDLTE(stationManagement.Ni,:));
        for j_neigh = indexNeighborOfVehicleTX          %发射车辆的相邻车辆才会有 接收功率
            % ID rx vehicle  %相邻矩阵的行 为发射端编号，列j_neigh为接收端编号，提取到IDrx
            IDrx = stationManagement.neighborsIDLTE(stationManagement.Ni,j_neigh);  
            ...
            %i→j的有效功率 Useful received power by vehicle j
            CGA = RXpower_MHz_ofLTEGA(stationManagement.activeIDsLTE==IDrx,stationManagement.transmittingIDsLTEGA(i_tx));
            IsumsGA = zeros(appParams.NbeaconsF,1);
            %% 排除Ni
            transmittingIDsLTEoverGA = stationManagement.transmittingIDsLTEGA;
%             [~, idx] = ismember(transmittingIDsLTEoverGA, stationManagement.Ni);
%             transmittingIDsLTEoverGA(idx > 0) = [];
            transmittingIDsLTEoverGA(transmittingIDsLTEoverGA == stationManagement.Ni) =[];
            if ~isempty(transmittingIDsLTEoverGA)
            %if Ntx > 1 % otherwise there is only one transmitter - no interference
                for k = 1:length(transmittingIDsLTEoverGA)  
                    if transmittingIDsLTEoverGA(k)~=IDrx
                        
                        BRFInt = BRidFGA(transmittingIDsLTEoverGA(k));
                        % Find power from interfering vehicle k receive by receiving vehicle j         
                        %IsumsGA(BRFInt,1) = IsumsGA(BRFInt,1) + IGA; %OMA
                        IGA = RXpower_MHz_ofLTEGA(stationManagement.activeIDsLTE==IDrx,transmittingIDsLTEoverGA(k));
                        if IGA < CGA   
                            IsumsGA(BRFInt,1) = IsumsGA(BRFInt,1) + IGA;
                        end
                    end
                end
            end

            ItotDataGA = phyParams.IBEmatrixData(BRFtx,:)*IsumsGA;
            ItotControlGA = phyParams.IBEmatrixControl(BRFtx,:)*IsumsGA;
            % Check if the receiver j is transmitting on the same BRT
            % of transmitter i
            if BRTtx==BRTrx
                selfI =Inf;     
            ...
            % SINR computation
            %SINR(i,j) = C / (PnRB + selfI + Itot);            
            sinrManagement.neighPowerUsefulLTEGA(i_tx,j_neigh) = CGA * phyParams.BwMHz_lteBR;%有效信号功率，然后用在updateSINRLTE里
            sinrManagement.neighPowerInterfDataLTEGA(i_tx,j_neigh) = (selfI + ItotDataGA) * phyParams.BwMHz_lteBR; %信号干扰功率
            sinrManagement.neighPowerInterfControlLTEGA(i_tx,j_neigh) = (selfI + ItotControlGA) * phyParams.BwMHz_lteBR;  %控制信息干扰功率
        end
    end
end